/**
 * ha-audio-card.js
 * Drop in /config/www/ha-audio-card.js
 * Edit directly in HA File Editor → refresh browser to update
 */
...
const haptic = (s = 'light') => {
  if ('vibrate' in navigator)
    navigator.vibrate({ light: [8], medium: [15], heavy: [25,8,25] }[s] || [8])
}

const svg = (d, sz = 24) =>
  `<svg width="${sz}" height="${sz}" viewBox="0 0 24 24" fill="currentColor"><path d="${d}"/></svg>`
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .linein-banner { display: flex; align-items: center; justify-content: space-between; padding: 12px 16px; border-radius: 14px; margin-bottom: 18px; background: rgba(255,200,60,0.1); border: 1px solid rgba(255,200,60,0.25); flex-shrink: 0; }
  ...
  .linein-btn:active { background: rgba(255,200,60,0.25); }

  .vinyl-toggle { display: flex; align-items: center; justify-content: space-between; padding: 11px 16px; border-radius: 14px; margin-bottom: 18px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.08); flex-shrink: 0; }
  ...
  .vinyl-switch-btn:active { background: rgba(255,255,255,0.14); }

  .track-row { display: flex; align-items: center; gap: 12px; margin-bottom: 16px; }
  ...
  .search-wrap { display: flex; align-items: center; gap: 10px; background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; padding: 0 14px; margin-bottom: 16px; flex-shrink: 0; transition: border-color 0.2s; }
  ...
  .provider-chips { display: flex; gap: 8px; margin-bottom: 16px; flex-shrink: 0; overflow-x: auto; }
  .pchip { display: flex; align-items: center; gap: 6px; padding: 7px 14px; border-radius: 999px; white-space: nowrap; font-size: 12px; font-weight: 600; background: rgba(255,255,255,0.07); border: 1px solid rgba(255,255,255,0.1); color: rgba(255,255,255,0.5); transition: all 0.18s; }
  .pchip.active { background: rgba(255,255,255,0.14); border-color: rgba(255,255,255,0.3); color: #fff; }
  .pchip:active { opacity: 0.7; }
  ...
  .list-item { display: flex; align-items: center; gap: 14px; padding: 10px 0; cursor: pointer; border-bottom: 1px solid rgba(255,255,255,0.05); transition: opacity 0.12s; }
...
class HaAudioCard extends HTMLElement {
  ...
  _syncBrowseView() {
    ...
    si?.addEventListener('input', e => {
      ...
      if (this._searchQuery.length > 1) {
        ...
        this._searchResults = null
        // Only update the body, not the whole view (preserves input focus)
        const body = this._root.getElementById('browse-body')
        if (body) body.innerHTML = this._renderRoot()
        this._bindBrowseBodyEvents()
      }
    })

    this._root.querySelectorAll('[data-browse]').forEach(b =>
      b.addEventListener('click', () => { haptic('light'); try { this._browseItem(JSON.parse(decodeURIComponent(b.dataset.browse))) } catch(e) { console.error('browse parse error', b.dataset.browse, e) } }))
    this._root.querySelectorAll('[data-play]').forEach(b =>
      b.addEventListener('click', e => { e.stopPropagation(); haptic('medium'); try { this._playItem(JSON.parse(decodeURIComponent(b.dataset.play))) } catch(e) { console.error('play parse error', b.dataset.play, e) } }))
  }
  ...
  _renderRoot() {
    ...
    for (const s of sections) {
      const bd = JSON.stringify({ contentId: s.id, contentType: 'music_assistant', label: s.label })
      html += '<div class="list-item" data-browse="' + encodeURIComponent(bd) + '">'
  ...
  _renderLevel(level) {
    if (!level || !level.items || !level.items.length) return '<div class="empty"><b>Nothing here</b>Try a different section</div>'
    const items = level.items
    const isTrack = items[0] && items[0].media_content_type === 'music' && !items[0].can_expand
    if (isTrack) {
      let html = '<div class="browse-list">'
      for (const item of items) {
        const ep = JSON.stringify({ contentId: item.media_content_id, contentType: item.media_content_type, name: item.title })
        ...
        html += '<button class="list-action" data-play="' + encodeURIComponent(ep) + '">&#9654;</button>'
        html += '</div>'
      }
      html += '</div>'
      return html
    ...
    for (const item of items) {
      const bd = JSON.stringify({ contentId: item.media_content_id, contentType: item.media_content_type, label: item.title })
      const ep = JSON.stringify({ contentId: item.media_content_id, contentType: item.media_content_type, name: item.title })
      html += '<div class="grid-item"' + (item.can_expand ? ' data-browse="' + encodeURIComponent(bd) + '"' : '') + '>'
      ...
      if (item.can_play) html += '<button class="list-action" style="margin-top:4px" data-play="' + encodeURIComponent(ep) + '">&#9654;</button>'
      html += '</div></div>'
    }
    html += '</div>'
    return html
  ...
  _renderSearch() {
    ...
    for (const [type, groupItems] of Object.entries(groups)) {
      ...
      for (const item of groupItems) {
        ...
        const bd = encodeURIComponent(JSON.stringify({
          contentId: item.uri,
          contentType: item.media_type || 'music_assistant',
          label: item.name,
          isUri: true,
        }))
        const ep = encodeURIComponent(JSON.stringify({
          contentId: item.uri,
          contentType: item.media_type || 'music',
          name: item.name,
          isUri: true,
        }))
        const thumb = item.image
          ? '<img src="' + this._esc(item.image) + '" loading="lazy" onerror="this.style.display=&quot;none&quot;">'
          : I.note()
  ...
  async _browseItem(data) {
    ...
    this._searchQuery = ''  // clear search so drill-down shows properly
    this._searchResults = null
    ...
    try {
      ...
      if (data.isUri) {
        // From search results - use the MA URI directly
        // For artists, append /albums to drill into their discography
        msg.media_content_id = data.contentId
        msg.media_content_type = 'music_assistant'
      } else {
        msg.media_content_id = data.contentId
        msg.media_content_type = data.contentType
      }
  ...
  async _doSearch() {
    ...
    try {
      ...
      // Flatten all results into a single list with type info
      const results = []
      for (const [type, items] of Object.entries(r)) {
        if (!Array.isArray(items)) continue
        for (const item of items) {
          results.push({ ...item, _resultType: type })
        }
      }
  ...
  _maConfigEntry() {
    // Get MA config entry ID from device registry
    const maEntity = Object.values(this._hass.entities || {})
      .find(e => e.platform === 'music_assistant')
    if (!maEntity) return ''
  ...
  _playItem(item) {
    ...
    haptic('medium')
    // Show room picker instead of playing immediately
  ...
  _playInRoom(room) {
    ...
    // Switch active room to where we're playing
    this._room = room.id
    this._pendingPlay = null
    this._roomPickerOpen = false
    this._closeSheet()
  ...
  _renderSheet() {
    ...
    bd.addEventListener('click',()=>this._closeSheet()); app.appendChild(bd)
    ...
    if (this._roomPickerOpen) {
      ...
      for (const r of ROOMS) {
        const s = this._hass && this._hass.states[r.sonos]
        const playing = s && s.state === 'playing'
        const vol = Math.round(((s && s.attributes && s.attributes.volume_level) || 0) * 100)
        const isActive = this._room === r.id
        const activeBg = isActive ? 'background:rgba(255,255,255,0.05);' : ''
        rowsHtml += '<div class="room-row" style="cursor:pointer;' + activeBg + '" data-room-play="' + r.id + '">'
        ...
        rowsHtml += '</div>'
        rowsHtml += '<div class="room-st">' + (playing ? 'Playing · ' + vol + '%' : (s && s.state === 'paused' ? 'Paused' : 'Idle')) + '</div>'
        rowsHtml += '</div><div style="color:rgba(255,255,255,0.4);font-size:20px">&#9654;</div></div>'
      }
    ...
    this._on('act-whole', ()=>{ haptic('medium'); this._call('sonos','join',{master:MASTER,entity_id:ALL_SONOS.filter(e=>e!==MASTER)}); this._closeSheet() })
    this._on('act-ung',   ()=>{ haptic('medium'); this._call('sonos','unjoin',{},{entity_id:ALL_SONOS}); this._closeSheet() })
    this._on('act-stop',  ()=>{ haptic('heavy');  this._call('media_player','media_stop',{},{entity_id:ALL_SONOS}); this._closeSheet() })
    this._on('act-cancel',()=>this._closeSheet())
  }

  _closeSheet() { this._queueOpen=false; this._groupOpen=false; this._renderSheet() }
  ...
  _ip(type)   { return {track:'M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z',album:'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 14.5c-2.49 0-4.5-2.01-4.5-4.5S9.51 7.5 12 7.5s4.5 2.01 4.5 4.5-2.01 4.5-4.5 4.5zm0-5.5c-.55 0-1 .45-1 1s.45 1 1 1 1-.45 1-1-.45-1-1-1z',artist:'M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z',playlist:'M15 6H3v2h12V6zm0 4H3v2h12v-2zM3 16h8v-2H3v2zM17 6v8.18c-.31-.11-.65-.18-1-.18-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3V8h3V6h-5z'}[type]||'M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z' }
  _eb(item)   { return JSON.stringify({uri:item.uri,name:item.name,media_type:item.media_type}).replace(/'/g,"&#39;") }
